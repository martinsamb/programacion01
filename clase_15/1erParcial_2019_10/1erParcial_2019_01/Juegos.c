...
#include <stdlib.h>
#include "utn.h"
#include "juegos.h" //cambiar por nombre entidad


/** \brief  To indicate that all position in the array are empty,
*          this function put the flag (isEmpty) in TRUE in all
*          position of the array
* \param array juego Array of juego
...
* \param array juego Array de juego
...
int juego_alta(Juego array[], int size, int* contadorID)                          //cambiar juego
{
    ...
    {
        if(juego_buscarEmpty(array,size,&posicion)==-1)                          //cambiar juego
        {
...
int juego_baja(Juego array[], int sizeArray)                                      //cambiar juego
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        if(juego_buscarID(array,sizeArray,id,&posicion)==-1)                                   //cambiar si no se busca por ID
        {
...
*/
/*
int juego_bajaValorRepetidoInt(Juego array[], int sizeArray, int valorBuscado) //cuando hay que dar de baja todas las posiciones en las que se encuentra ese int
{
    int retorno=-1;
    int i;
    if(array!=NULL && sizeArray>0)
...
int juego_ordenarPorString(Juego array[],int size)                              //cambiar juego
{
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            strcpy(bufferString,array[i].Descripcion);                      //cambiar campo varString
            bufferId=array[i].idUnico;                                   //cambiar campo id
            bufferIsEmpty=array[i].isEmpty;

            bufferFloat=array[i].Importe;                            //cambiar campo varFloat


            j = i - 1;
            while ((j >= 0) && strcmp(bufferString,array[j].Descripcion)<0)         //cambiar campo varString                 //Si tiene mas de un criterio se lo agrego, Ej. bufferInt<array[j].varInt
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                strcpy(array[j + 1].Descripcion,array[j].Descripcion);          //cambiar campo varString
                array[j + 1].idUnico=array[j].idUnico;                                //cambiar campo id
                array[j + 1].isEmpty=array[j].isEmpty;

                array[j + 1].Importe=array[j].Importe;                    //cambiar campo varFloat

                j--;
            }
            strcpy(array[j + 1].Descripcion,bufferString);                     //cambiar campo varString
            array[j + 1].idUnico=bufferId;                                        //cambiar campo id
            array[j + 1].isEmpty=bufferIsEmpty;

            array[j + 1].Importe=bufferFloat;                                                    //cambiar campo varFloat
        }
        retorno=0;
    }
    return retorno;
