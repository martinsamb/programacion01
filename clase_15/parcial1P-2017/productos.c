...
int producto_buscarID(Producto array[], int size, int valorBuscado, int* posicion)                    //cambiar producto
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].idProducto==valorBuscado)                                                   //cambiar campo ID
            {
                retorno=0;
                *posicion=i;
                break;
...
int producto_alta(Producto array[], int size, int* contadorID)                          //cambiar producto
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        {
            (*contadorID)++;
            array[posicion].idProducto=*contadorID;                                                       //campo ID
            array[posicion].isEmpty=0;
            utn_getTexto("\nNombre: ","\nError",1,TEXT_SIZE,1,array[posicion].nombre);                 //mensaje + cambiar campo varLongString
            utn_getUnsignedInt("\nID Usuario: ","\nError",1,sizeof(int),1,1,1,&array[posicion].idUsuario);           //mensaje + cambiar campo varInt
            utn_getFloat("\nPrecio: ","\nError",1,sizeof(float),0,1,1,&array[posicion].precio);             //mensaje + cambiar campo varFloat
            utn_getUnsignedInt("\nStock: ","\nError",1,sizeof(int),1,1,1,&array[posicion].stock);           //mensaje + cambiar campo varInt
            utn_getUnsignedInt("\nCantidadVendida: ","\nError",1,sizeof(int),1,1,1,&array[posicion].cantidadVendida);           //mensaje + cambiar campo varInt
            utn_getUnsignedInt("\nEstado: ","\nError",1,sizeof(int),1,1,1,&array[posicion].estado);           //mensaje + cambiar campo varInt
            printf("\nNombre: %s\nID Usuario: %d\nPrecio: %.2f\nStock: %d\nCantidadVendida: %d\nEstado: %d",
                   array[posicion].idProducto,
                   array[posicion].nombre,
                   array[posicion].idUsuario,
                   array[posicion].precio,
                   array[posicion].stock,
                   array[posicion].cantidadVendida,
                   array[posicion].estado);
            retorno=0;
        }
    }
    return retorno;
...
int producto_baja(Producto array[], int sizeArray)                                      //cambiar producto
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].idProducto=0;                                                                   //cambiar campo id
            array[posicion].estado=0;                                                               //cambiar campo varInt
            array[posicion].precio=0;
            array[posicion].stock=0;
            array[posicion].cantidadVendida=0;                                                             //cambiar campo varFloat
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            {       //copiar printf de alta
                ...
                {
                    case 'A':
                        utn_getUnsignedInt("\nId Producto: ","\nError",1,sizeof(int),1,1,1,&array[posicion].idProducto);           //mensaje + cambiar campo varInt
                        break;
                    case 'B':
                        utn_getFloat("\nPrecio: ","\nError",1,sizeof(float),0,1,1,&array[posicion].precio);             //mensaje + cambiar campo varFloat
                        break;
